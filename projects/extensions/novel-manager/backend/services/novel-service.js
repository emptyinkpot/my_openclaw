...
const ContentPipeline_1 = require("../../core/content-pipeline/ContentPipeline");
const FanqieSimplePipeline_1 = require("../../core/publishing/FanqieSimplePipeline");
const ProgressManager_1 = require("../../core/pipeline/ProgressManager");
function resolveWorkspaceRoot() {
    const browserDir = (0, config_1.getConfig)().scheduler?.fanqieAccounts?.[0]?.browserDir;
    if (browserDir) {
...
function getExperienceDataPath() {
    return path.join(resolveWorkspaceRoot(), 'extensions', 'experience-manager', 'data', 'experiences.json');
}
function getStorageDir() {
    return ensureDir(path.join(resolveWorkspaceRoot(), 'workspace', 'storage'));
}
function getScreenshotDir() {
    return ensureDir(path.join(resolveWorkspaceRoot(), 'workspace', 'screenshots'));
}
class NovelService {
    constructor() {
    ...
    getPipeline() {
        if (!this.contentPipeline) {
            this.contentPipeline = new ContentPipeline_1.ContentPipeline();
        }
    ...
    async initTables() {
      ...
      `).catch(() => { });
            ...
            try {
                ...
                const titleManager = (0, database_1.getChapterTitleManager)();
    ...
    async getChaptersByWorkId(workId) {
      ...
      SELECT id, chapter_number, title, word_count, status, 
             polish_status, polished_content, polished_at,
     ...
     * 获取作品的故事背景
     */
    async getStoryBackgroundByWorkId(workId) {
        return await this.db.queryOne(`
     ...
     * 获取作品的大纲
     */
    async getOutlineByWorkId(workId) {
        return await this.db.queryOne(`
     ...
     * 获取关联章节（前面的章节）
     */
    async getRelatedChapters(workId, chapterNumber, count = 2) {
    ...
    async updateChapter(id, data) {
        ...
        const params = [];
        // 获取当前章节数据
        const currentChapter = await this.db.queryOne('SELECT * FROM chapters WHERE id = ?', [id]);
        if (data.title !== undefined) {
            updates.push('title = ?');
            params.push(data.title);
        }
     ...
     * 使用状态机更新章节状态（推荐方式）
     */
    async updateChapterWithStateMachine(id, toState, reason, data) {
        // 先更新标题和内容（如果有）
        if (data) {
    ...
    */
    ...
    async getExperienceRecords() {
      ...
      `);
        ...
        const expPath = getExperienceDataPath();
        try {
    ...
    async testBrowser() {
        ...
        try {
            ...
            return {
                success: true,
                message: `测试成功，截图已保存到 ${screenshotPath}`,
    ...
    async startPipeline(options) {
        ...
        const pipeline = new FanqieSimplePipeline_1.FanqieSimplePipeline();
        try {
            // 生成 progressId
            const progressId = `pipeline-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
            console.log('[NovelService] 生成 progressId:', progressId);
            // 立即返回响应，后台运行任务
            (async () => {
                try {
                    const workId = options?.workId ?? 9;
                    ...
                    console.error('[NovelService] 流水线失败:', err);
                    console.error('[NovelService] 错误堆栈:', err.stack);
                }
            })();
            return {
                success: true,
                message: '流水线已启动，正在后台运行...',
