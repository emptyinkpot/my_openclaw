...
async function getProjectStructure() {
  ...
  // 获取模块内前端页面列表（已不再依赖 extensions/public）
...
function applyLiteralReplacements(
  ...
  for (const row of sortedRows) {
    ...
    if (!seen.has(key)) {
      ...
      replacements.push({
        original,
        replaced: replacement || '(已移除)',
        reason: row.reason || options?.defaultReason || '本地兜底替换',
    ...
    if (path === '/api/novel/banned-words-replace' && method === 'POST') {
      ...
      try {
        ...
        const fallbackResult = applyLiteralReplacements(text, matchedBannedWords, {
          source: 'MySQL禁用词库本地兜底',
          defaultReason: '禁用词本地兜底替换'
        });
...
请直接返回替换后的完整文本，不要添加任何解释或说明。`;
            ...
            if (llmText) {
              ...
              replacements = matchedBannedWords
                ...
                .map((item: any) => ({
                  original: item.word,
                  replaced: item.replacement || '(智能替换)',
                  reason: item.reason || '禁用词替换',
                  source: 'MySQL禁用词库 + LLM智能替换'
                }));
              mode = 'llm';
            }
          }
