...
exports.FanqiePublisher = void 0;
...
const ChapterRepository_1 = require("./ChapterRepository");
class FanqiePublisher {
     ...
     */
    async getPendingChapters(workId, limit = 10) {
     ...
     * 发布单章到番茄
     */
    async publishChapter(chapter, account, options = {}) {
        const { dryRun = false, headless = false, onProgress } = options;
        const emitProgress = (action, detail) => {
            logger_1.logger.info(`  → ${action}`);
            onProgress?.({ action, detail });
        };
        logger_1.logger.info(`[番茄发布] 开始发布: ${chapter.workTitle} 第${chapter.chapterNumber}章`);
        if (dryRun) {
            emitProgress('模拟发布模式');
            logger_1.logger.info(`[DRY RUN] 将发布到账号: ${account.name}`);
            return {
                success: true,
        ...
        try {
            ...
            logger_1.logger.info(`  番茄最新章节: 第${latestChapter}章`);
            // 3. 确认章节序号连续
     ...
     * 初始化浏览器
     */
    async initBrowser(account, headless) {
        // 清理锁文件
        this.cleanBrowserLocks(account.browserDir);
        this.browserContext = await playwright_1.chromium.launchPersistentContext(account.browserDir, {
            headless,
            channel: 'chrome',
        ...
        if (fs.existsSync(account.cookiesFile)) {
            try {
                ...
                logger_1.logger.warn('加载cookies失败:', e);
            }
        }
    }
    /**
     ...
     * 在番茄找到对应作品
     */
    async findFanqieWork(workTitle) {
        if (!this.page)
            return null;
        ...
        // 匹配作品（精确匹配或包含匹配）
        logger_1.logger.debug(`  番茄网站上的作品列表: ${JSON.stringify(works)}`);
        logger_1.logger.info(`  查找作品: "${workTitle}"`);
        for (const work of works) {
            logger_1.logger.debug(`    尝试匹配: "${work.title}" vs "${workTitle}"`);
            if (work.title === workTitle) {
                logger_1.logger.info(`    精确匹配成功: ${work.title}`);
                return work;
            }
            if (work.title.includes(workTitle) || workTitle.includes(work.title)) {
                logger_1.logger.info(`    包含匹配成功: ${work.title}`);
                return work;
            }
            // 清理标点符号后比较
            ...
            if (clean1 === clean2) {
                logger_1.logger.info(`    清理后匹配成功: ${work.title} -> ${clean1}`);
                return work;
            }
            // 去除"征文作品"等常见后缀后比较
            ...
            if (normalized1 === normalized2 || normalized1.includes(normalized2) || normalized2.includes(normalized1)) {
                logger_1.logger.info(`    标准化匹配成功: ${work.title}`);
                return work;
            }
        }
        logger_1.logger.error(`  未找到匹配作品。可用作品: ${works.map(w => `"${w.title}"`).join(', ')}`);
        return null;
    }
    /**
     * 获取番茄作品的最新章节号
     */
    async getFanqieLatestChapter(workId) {
        if (!this.page)
            return 0;
        // 进入章节管理
        ...
        // 解析章节号（支持阿拉伯数字和中文数字）
        logger_1.logger.debug(`  章节文本列表: ${JSON.stringify(chapterTexts.slice(0, 10))}`);
        let maxChapter = 0;
        for (const text of chapterTexts) {
            const num = (0, text_1.extractChapterNumber)(text);
            logger_1.logger.debug(`    解析 "${text.trim()}": ${num}`);
            if (num !== null && num > maxChapter) {
                maxChapter = num;
            }
        }
        logger_1.logger.info(`  解析到最新章节: ${maxChapter}`);
        return maxChapter;
    }
    /**
     * 公开方法：获取番茄作品的最新章节号（独立调用）
     */
    async getLatestChapterFromFanqie(workTitle, account, headless = true) {
        try {
            // 初始化浏览器
            logger_1.logger.info(`[FanqiePublisher] 获取最新章节: ${workTitle}`);
            await this.initBrowser(account, headless);
            // 找到作品
            const fanqieWork = await this.findFanqieWork(workTitle);
            if (!fanqieWork) {
                logger_1.logger.error(`  未找到作品: ${workTitle}`);
                return null;
            }
            // 获取最新章节
            const latestChapter = await this.getFanqieLatestChapter(fanqieWork.id);
            logger_1.logger.info(`  最新章节: ${latestChapter}`);
            return latestChapter;
        }
        catch (error) {
            logger_1.logger.error('获取最新章节失败:', error);
            return null;
        }
        finally {
            await this.closeBrowser();
     ...
     * 执行发布
     */
    async doPublishToFanqie(workId, chapter) {
        if (!this.page) {
            throw new Error('浏览器未初始化');
        }
        logger_1.logger.info('  → 点击创建章节...');
        // 等待页面完全加载
        ...
        logger_1.logger.debug('    已保存点击前截图: /tmp/fanqie-before-click.png');
        // 调试：点击前的页面信息
        ...
        logger_1.logger.debug(`    点击前: ${JSON.stringify(beforeClick)}`);
        // 尝试多种方式点击"新建章节"按钮/链接
        let clicked = false;
        // 方式1: 直接获取 A 标签的 href 并导航
        try {
            const href = await this.page.evaluate(() => {
                const link = Array.from(document.querySelectorAll('a')).find(a => a.textContent?.trim() === '新建章节');
                return link?.getAttribute('href') || null;
            });
            if (href) {
                logger_1.logger.debug(`    从 A 标签获取到 href: ${href}`);
                // 如果是相对路径，转换为绝对路径
                const fullUrl = href.startsWith('http') ? href : `https://fanqienovel.com${href}`;
                await this.page.goto(fullUrl, { waitUntil: 'networkidle' });
                logger_1.logger.debug(`    直接导航到新建章节页面`);
                clicked = true;
            }
        }
        catch (e) {
            logger_1.logger.debug(`    直接导航失败: ${e.message}`);
        }
        // 方式2: 优先点击 A 标签
        if (!clicked) {
            try {
                ...
                if (await linkBtn.count() > 0 && await linkBtn.isVisible()) {
                    ...
                    logger_1.logger.debug(`    使用 A 标签点击成功`);
                    clicked = true;
                }
            }
            catch (e) {
                logger_1.logger.debug(`    A 标签点击失败: ${e.message}`);
            }
        }
        ...
        if (!clicked) {
            try {
                ...
                if (await btn.count() > 0 && await btn.isVisible()) {
                    ...
                    logger_1.logger.debug(`    使用 getByRole 点击成功`);
                    clicked = true;
                }
            }
            catch (e) {
                logger_1.logger.debug(`    getByRole 点击失败: ${e.message}`);
            }
        }
        // 方式4: 通过 evaluate 点击
        if (!clicked) {
            try {
                const result = await this.page.evaluate(() => {
                    ...
                    const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent?.trim() === '新建章节');
                    if (btn) {
                        btn.click();
                ...
                logger_1.logger.debug(`    evaluate 点击结果: ${result}`);
                if (result !== 'not found')
                    clicked = true;
            }
            catch (e) {
                logger_1.logger.debug(`    evaluate 点击失败: ${e.message}`);
            }
        }
        if (!clicked) {
            logger_1.logger.error('    所有点击方式都失败');
            throw new Error('无法点击"新建章节"按钮');
        }
        // 等待页面变化（条件等待：等待输入框/编辑器/URL变化）
        logger_1.logger.info('    等待页面响应...');
        try {
            // 优化：多种条件等待，哪个先满足就继续
            await Promise.race([
                // 等待输入框或编辑器出现
            ...
            logger_1.logger.debug('    页面响应条件满足（输入框/URL变化）');
        }
        catch (e) {
            logger_1.logger.debug('    条件等待超时，继续执行');
        }
        // 保存点击后截图
        ...
        logger_1.logger.debug(`    点击后: ${JSON.stringify(afterClick)}`);
        // 如果页面没有输入框和编辑器，可能需要先进入正确的页面
        if (afterClick.inputs === 0 && afterClick.editors === 0) {
            logger_1.logger.warn('    页面没有输入框，尝试进入章节管理页面...');
            // 尝试点击"章节管理"按钮
            try {
                ...
                if (await manageBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
                    ...
                    logger_1.logger.debug(`    点击"章节管理"按钮`);
                    // 条件等待章节列表加载
                ...
                logger_1.logger.debug(`    章节管理按钮未找到: ${e.message}`);
            }
            // 再次尝试点击新建章节
            const retryResult = await this.page.evaluate(() => {
                const btn = Array.from(document.querySelectorAll('button, a')).find(b => b.textContent?.includes('新建章节') || b.textContent?.includes('创建章节'));
                if (btn) {
                    btn.click();
            ...
            logger_1.logger.debug(`    重试点击结果: ${retryResult}`);
            // 条件等待输入框出现
            try {
        ...
        logger_1.logger.debug(`  页面输入框: ${JSON.stringify(inputDebug, null, 2)}`);
        // 填充章节号（如：第50章）
        logger_1.logger.info(`  → 填写章节号: 第${chapter.chapterNumber}章`);
        // 先获取所有输入框的详细信息
        ...
        logger_1.logger.debug(`    所有input: ${JSON.stringify(allInputs)}`);
        ...
        if (numberInputs.length > 0) {
            ...
            logger_1.logger.debug(`    章节号使用第${idx}个number类型input`);
        ...
        if (!chapterNumFilled) {
            ...
            for (const selector of chapterNumSelectors) {
                ...
                if (await input.isVisible().catch(() => false)) {
                    await input.fill(chapter.chapterNumber.toString());
                    logger_1.logger.debug(`    章节号使用选择器: ${selector}`);
                    chapterNumFilled = true;
                    break;
                }
            }
        ...
        if (!chapterNumFilled) {
            ...
            await input.fill(chapter.chapterNumber.toString());
            logger_1.logger.debug(`    章节号使用第0个input: ${chapter.chapterNumber}`);
            chapterNumFilled = true;
        }
        if (!chapterNumFilled) {
            logger_1.logger.error(`    无法找到章节号输入框`);
        }
        else {
            // 验证填写（短延时等待值生效）
            ...
            logger_1.logger.debug(`    验证章节号: ${verifyChapterNum}`);
        }
        // 填充章节标题
        logger_1.logger.info(`  → 填写标题: ${chapter.chapterTitle}`);
        // 查找标题输入框
        ...
        for (const selector of titleSelectors) {
            ...
            if (await input.isVisible().catch(() => false)) {
                ...
                logger_1.logger.debug(`    标题使用选择器: ${selector}`);
                titleFilled = true;
                break;
            }
        }
        if (!titleFilled) {
            logger_1.logger.warn(`    未找到标题输入框，尝试第1个input`);
        ...
        // 填充正文内容
        logger_1.logger.info(`  → 填写正文 (${chapter.wordCount}字)...`);
        // 清理内容
        const cleanedContent = this.cleanContent(chapter.content);
        logger_1.logger.debug(`    正文前100字: ${cleanedContent.slice(0, 100)}`);
        // 查找编辑器
        ...
        for (const selector of editorSelectors) {
            ...
            if (await editor.isVisible().catch(() => false)) {
                ...
                logger_1.logger.debug(`    正文使用选择器: ${selector} (evaluate方式)`);
                editorFilled = true;
                break;
            }
        }
        if (!editorFilled) {
            logger_1.logger.warn(`    未找到编辑器，尝试第一个 textarea 或 contenteditable`);
            ...
            if (await fallbackEditor.isVisible().catch(() => false)) {
                ...
                logger_1.logger.error(`    无法找到任何编辑器！`);
            }
        }
        // 验证填入内容
            ...
            for (const pattern of submitPatterns) {
                try {
                    ...
                    if (await submitBtn.count() > 0 && await submitBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
                        ...
                        logger_1.logger.debug(`    点击匹配按钮成功 (${pattern.toString()}`);
                        submitClicked = true;
                        break;
                    }
                }
                catch (e) {
                    logger_1.logger.debug(`    尝试按钮${pattern}失败: ${e.message}`);
                }
            }
        }
        if (!submitClicked) {
            logger_1.logger.debug('    至少有一个提交按钮被点击');
        }
        else {
            logger_1.logger.warn('    所有提交按钮尝试失败');
        }
        ...
        while (retryCount < 3 && !publishSuccess) {
            try {
                ...
                logger_1.logger.debug('    点击"确认发布"成功');
                publishSuccess = true;
            }
            catch (e) {
                retryCount++;
                logger_1.logger.warn(`    第${retryCount}次点击"确认发布"失败: ${e.message}`);
                if (retryCount >= 3)
                    break;
            }
        }
        // 条件等待：检测页面跳转
        logger_1.logger.debug('    等待发布完成...');
        ...
        logger_1.logger.debug(`    最终页面信息: ${JSON.stringify(pageInfo)}`);
        // 严格验证：检查是否真的发布了章节
        ...
        if (success) {
            logger_1.logger.info('  ✓ 发布成功提示已出现');
            // 增强验证：回到章节管理页，检查最新章节号是否真的更新为 chapter.chapterNumber
            logger_1.logger.info('  → 验证发布结果：回到章节管理页检查...');
            try {
                ...
                if (verifyFanqieWork) {
                    ...
                    if (verifyLatest === chapter.chapterNumber) {
                        logger_1.logger.info(`  ✓ 验证成功：最新章节号确实是 ${verifyLatest}`);
                    }
                    else {
                        logger_1.logger.warn(`  ⚠ 验证结果：最新章节号是 ${verifyLatest}，期望 ${chapter.chapterNumber}`);
                    }
                }
            }
            catch (verifyErr) {
                logger_1.logger.warn(`  ⚠ 验证步骤失败，不影响发布结果: ${verifyErr.message}`);
            }
            return {
                success: true,
                workId: chapter.workId,
            ...
            logger_1.logger.error(`  发布失败截图已保存: ${screenshotPath}`);
     ...
     * 更快的检测，更短的超时
     */
    async waitForAutoSave(timeout = 6000) {
        if (!this.page)
            return;
        ...
        while (Date.now() - startTime < timeout) {
            ...
            if (isSaved === 'explicit_success') {
                logger_1.logger.debug('    自动保存完成（明确成功提示）');
                return;
            }
            if (isSaved === 'no_indicator') {
                if (!firstSuccessTime) {
                    ...
                    logger_1.logger.debug('    自动保存完成（无保存指示器）');
                    return;
                }
            }
            else {
        ...
        logger_1.logger.debug('    等待自动保存超时，继续执行（优化版）');
    }
    /**
     * 条件等待：等待弹窗出现
     */
     ...
     * 条件等待：等待弹窗消失
     */
    async waitForDialogClosed(timeout = 3000) {
        if (!this.page)
            return;
        try {
            await this.page.waitForSelector('.arco-modal-wrapper, .ant-modal, [role="dialog"], .modal, div[class*="modal"], div[class*="dialog"]', {
     ...
     * 条件等待：等待页面跳转
     */
    async waitForNavigation(targetUrlPattern, timeout = 10000) {
        if (!this.page)
            return false;
        const startTime = Date.now();
        ...
        while (Date.now() - startTime < timeout) {
            ...
            if (currentUrl.includes(targetUrlPattern)) {
                logger_1.logger.debug(`    页面已跳转到: ${currentUrl}`);
                return true;
            }
            await this.page.waitForTimeout(checkInterval);
        }
...
exports.FanqiePublisher = FanqiePublisher;
//# sourceMappingURL=FanqiePublisher.js.map