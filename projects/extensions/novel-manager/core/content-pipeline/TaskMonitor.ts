...
export class TaskMonitor {
  private externalErrorKeywords: string[] = [
  ...
  markNotified(workName: string, action: string, chapterNum: number): void {
    const task = this.failedTasks.find(t => 
      t.workName === workName && 
      t.action === action && 
  ...
  getStats(): { total: number; external: number; critical: number; internal: number } {
    return {
  ...
  private summarizeError(errorMessage: string): string {
    if (!errorMessage) return '未知错误';
    
    const msg = errorMessage.substring(0, 200).toLowerCase();
    
    if (msg.includes('timeout')) return '超时';
    if (msg.includes('econnrefused') || msg.includes('econnreset')) return '网络连接失败';
    if (msg.includes('target page, context or browser has been closed')) return '浏览器会话关闭';
    if (msg.includes('not found') || msg.includes('404')) return '资源未找到';
    if (msg.includes('authentication') || msg.includes('unauthorized') || msg.includes('401')) return '认证失败';
    if (msg.includes('permission') || msg.includes('forbidden') || msg.includes('403')) return '权限不足';
    if (msg.includes('element is not stable')) return '页面元素不稳定';
    if (msg.includes('intercepts pointer events')) return '元素被遮挡';
