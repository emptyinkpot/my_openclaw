...
const OUTPUT_ROOT = process.env.MEMORY_LANCEDB_PRO_MIRROR_DIR
  ? path.resolve(process.env.MEMORY_LANCEDB_PRO_MIRROR_DIR)
  : path.resolve(projectRoot, "..", "memory-lancedb-pro", "mirror", "experience-manager");
...
async function writeIndex(experienceCount, noteCount) {
  ...
  const readme = [
    "# Memory LanceDB Pro Mirror",
    "",
    "This mirror is generated from the cloud-backed experience manager and is used by the Codex memory bridge.",
    "",
    `- Experience records: ${experienceCount}`,
    `- Notes: ${noteCount}`,
    "",
    "Refresh it from the experience-manager plugin after record_experience or record_note.",
  ].join("\n");
...
main().catch((error) => {
  console.error("[sync-to-memory-lancedb-pro] fatal error:", error);
  process.exitCode = 1;
});
