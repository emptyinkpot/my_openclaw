...
TEMP_ROOT="${TMPDIR:-${TEMP:-${TEMP_DIR:-/tmp}}}"

# 经验数据文件路径
EXPERIENCE_FILE="${EXPERIENCE_FILE:-$REPO_ROOT/projects/extensions/experience-manager/data/experiences.json}"
TEMP_EXPERIENCE_FILE=$(mktemp "$TEMP_ROOT/auto-experience.XXXXXX.json")
trap 'rm -f "$TEMP_EXPERIENCE_FILE"' EXIT
...
if echo "$COMMIT_MSG" | grep -qE "(fix|feat|refactor|docs):"; then
    ...
    mkdir -p "$(dirname "$EXPERIENCE_FILE")"

    # 创建经验记录JSON
    cat > "$TEMP_EXPERIENCE_FILE" << EOF
    {
      "type": "${EXP_TYPE}",
      "title": "${COMMIT_MSG#*: }",
...
EOF
    ...
    echo "  临时记录文件: ${TEMP_EXPERIENCE_FILE}"
    echo "  目标经验库: ${EXPERIENCE_FILE}"
fi
