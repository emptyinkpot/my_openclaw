...
function getMemoryDir(): string {
  return process.env.MEMORY_LANCEDB_PATH || path.join(resolveWorkspaceRoot(), 'data', 'memory-lancedb');
}
...
export async function syncExperienceToMemory(record: ExperienceRecord): Promise<boolean> {
  ...
  try {
    ...
    await fsPromises.appendFile(syncFile, JSON.stringify(syncEntry) + '\n');
...
export async function searchRelatedExperiences(query: string, limit: number = 5): Promise<Array<{ text: string; score: number; metadata?: any }>> {
  ...
  try {
    ...
    // 读取同步队列文件
    const content = await fsPromises.readFile(syncFile, 'utf-8').catch(() => '');
    if (!content.trim()) {
      return [];
    }
