...
const NAV_FILE = path.join(PROJECT_ROOT, 'shared', 'nav-bar.html');

function readNavBarHtml(): string {
  try {
    return fs.readFileSync(NAV_FILE, 'utf-8');
  } catch (error) {
    console.error('[experience-manager] 读取导航栏失败:', error);
    return '<div class="nav-bar"><a href="/experience" class="on">经验中心</a></div>';
  }
}

function buildInjectedNavBar(): string {
  let navBarHtml = readNavBarHtml();
  navBarHtml = navBarHtml.replace('href="/experience"', 'href="/experience" class="on"');
...
`;
  ...
  return navBarHtml + dynamicHeightScript;
}

function renderExperiencePage(): string {
  try {
    const html = fs.readFileSync(PAGE_FILE, 'utf-8');
    const injectedNavBar = buildInjectedNavBar();
    const navBarRegex = /<div[^>]*class="[^"]*nav-bar[^"]*"[^>]*>[\s\S]*?<\/div>/;

    if (navBarRegex.test(html)) {
      return html.replace(navBarRegex, injectedNavBar);
    }

    if (html.includes('<body>')) {
      return html.replace('<body>', `<body>${injectedNavBar}`);
    }

    return injectedNavBar + html;
  } catch (error) {
