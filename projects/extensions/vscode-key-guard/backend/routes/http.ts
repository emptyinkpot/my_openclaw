...
const VSCODE_KEY_GUARD_CANONICAL_PATH = '/key-guard/chat';
...
const SHARED_NAV_BAR_SOURCE = path.join(moduleDir, '..', '..', '..', '..', 'shared', 'nav-bar.html');
...
function injectNavBar(html: string): string {
  const fallbackNav = '<div class="nav-bar"><a href="/key-guard/chat" class="on">Key Guard</a></div>';
  const navBarHtml = readUtf8File(SHARED_NAV_BAR_SOURCE, fallbackNav).replace(
    'href="/key-guard/chat"',
    'href="/key-guard/chat" class="on"'
  );
...
export async function handleVscodeKeyGuardPage(
  ...
  if (!VSCODE_KEY_GUARD_PAGE_ROUTES.includes(routePath as (typeof VSCODE_KEY_GUARD_PAGE_ROUTES)[number])) {
    return false;
  }

  if (routePath === '/key-guard' || routePath === '/key-guard/') {
    res.writeHead(302, {
