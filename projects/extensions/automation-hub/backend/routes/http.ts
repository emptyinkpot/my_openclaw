...
function injectNavBar(html: string): string {
  const navBarHtml = readUtf8File(
    SHARED_NAV_BAR_SOURCE,
    '<div class="nav-bar"><a href="/automation" class="on">自动化</a></div>'
  ).replace('href="/automation"', 'href="/automation" class="on"');

  if (html.includes('<div id="navbar-container"></div>')) {
    return html.replace(
      '<div id="navbar-container"></div>',
      `<div id="navbar-container">${navBarHtml}</div>`
    );
  }
