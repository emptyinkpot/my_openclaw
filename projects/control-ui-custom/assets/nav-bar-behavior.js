(function () {
  ...
  function applyActiveState() {
    ...
    navItems.forEach((item) => {
      ...
      if (href === 'javascript:void(0)' && currentPath.includes('settings')) {
        ...
        item.classList.add('on');
      } else if (href === '/novel/' && (currentPath === '/novel' || currentPath === '/novel/')) {
        item.classList.add('on');
  ...
  async function injectSharedNavBarIfNeeded() {
    const container = document.getElementById('navbar-container');
    if (!container) return;

    if (container.querySelector('.nav-bar')) {
      applyActiveState();
      return;
    }

    try {
      const response = await fetch(sharedNavUrl, { cache: 'no-store' });
      if (!response.ok) return;

      container.innerHTML = await response.text();
      applyActiveState();
    } catch (error) {
      console.error('[openclaw-nav] failed to inject shared nav bar:', error);
    }
  }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      void injectSharedNavBarIfNeeded();
    }, { once: true });
  } else {
    void injectSharedNavBarIfNeeded();
  }
