...
function Read-StartupState {
  try {
    if (-not (Test-Path -LiteralPath $StartupStatePath)) {
      return $null
    }

    return (Get-Content -LiteralPath $StartupStatePath -Raw | ConvertFrom-Json)
  } catch {
    return $null
  }
}

function Test-StaleStartupState {
  param(
    [Parameter(Mandatory = $true)]
    [int]$Port,
    [int]$AgeThresholdSec = 45
  )
  ...
  if ($state.port -and ([int]$state.port -ne $Port)) {
    return $false
  }

  $staleStatuses = @('waiting_for_health', 'warming_up', 'warming_existing', 'health_timeout', 'warming_up_pending_health')
  if (-not $staleStatuses.Contains([string]$state.status)) {
    return $false
  }
  ...
  $ageSec = [int]([datetimeoffset]::Now - $updatedAt).TotalSeconds
  return ($ageSec -ge $AgeThresholdSec)
}
...
function Get-GatewayWarmupInfo {
  param(
    [Parameter(Mandatory = $true)]
    [int]$Port,
    [int]$WarmupWindowSec = 180
  )
  ...
  $youngestProcess = $gatewayProcesses |
    Sort-Object CreationDate -Descending |
    Select-Object -First 1

  $ageSec = [math]::Max(0, [int](((Get-Date) - $youngestProcess.CreationDate).TotalSeconds))
  return [pscustomobject]@{
    Processes = $gatewayProcesses
    ProcessCount = $gatewayProcesses.Count
    AgeSec = $ageSec
    IsWarm = ($ageSec -lt $WarmupWindowSec)
  }
}
...
function Open-Browser {
  ...
  if ($env:OPENCLAW_SKIP_BROWSER -eq '1') {
    Write-Output "Gateway is healthy on 127.0.0.1:$Port. Browser launch skipped because OPENCLAW_SKIP_BROWSER=1."
    return
  }
  ...
  if ($startPath -eq '/control-ui-custom/index.html') {
    $browserUrl = "http://127.0.0.1:$Port$startPath#token=$([uri]::EscapeDataString($GatewayToken))&gatewayUrl=$([uri]::EscapeDataString($gatewayUrl))"
  } else {
...
function Start-BrowserLauncher {
  ...
  $launcherArgs = @(
    ...
    $Port,
    '-StartPath',
    (Quote-ProcessArgument $StartPath),
    '-TimeoutSec',
    $TimeoutSec
  )
...
try {
  ...
  if (-not (Acquire-LaunchMutex -TimeoutMs 1500)) {
    ...
    Start-BrowserLauncher -Port $launchPort -TimeoutSec 180
  ...
  if (-not $ForceRestart) {
    if (Test-StaleStartupState -Port $launchPort -AgeThresholdSec 45) {
      $recoverFromStaleState = $true
      Write-StartupState -Status 'stale_state_recovery' -Message 'Detected stale startup state. Performing self-healing restart.' -Port $launchPort
    }
    ...
    $warmupInfo = Get-GatewayWarmupInfo -Port $launchPort -WarmupWindowSec 180
    if ((-not $recoverFromStaleState) -and $warmupInfo.IsWarm) {
      if (Wait-ForGatewayHealthy -Port $launchPort -TimeoutSec 5) {
        Write-StartupState -Status 'healthy_after_short_wait' -Message 'Existing gateway became healthy during the short foreground wait.' -Port $launchPort
        Open-Browser -GatewayToken $Token -Port $launchPort
        exit 0
      }

      Write-StartupState -Status 'warming_existing' -Message ("Existing gateway launch is still warming up ({0} tracked process(es), age {1}s)." -f $warmupInfo.ProcessCount, $warmupInfo.AgeSec) -Port $launchPort
      Start-BrowserLauncher -Port $launchPort -TimeoutSec 180
      Write-Output "OpenClaw is already starting in the background. The browser will open automatically when the existing launch becomes healthy."
      exit 0
    }
  ...
  $shouldStopExistingGateway = $ForceRestart -or $recoverFromStaleState -or (Test-PortListening -Port $launchPort)
