...
if not exist "%CONFIG%" (
  ...
  if not exist "%CONFIG%" set "CONFIG=%~dp0projects\openclaw.example.json"
)
...
if not exist "%OPENCLAW_CONFIG_PATH%" (
  ...
  echo.
  echo Restore the runtime files under "%~dp0.local\openclaw" or use "%~dp0projects\openclaw.example.json" as your starting template.
  pause
  exit /b 1
)
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0openclaw-gateway-run.ps1" -ConfigPath "%OPENCLAW_CONFIG_PATH%" -Token "%OPENCLAW_GATEWAY_TOKEN%" -Root "%ROOT%" -LogPath "%GATEWAY_LOG%" %FORCE_ARGS%
if errorlevel 1 (
