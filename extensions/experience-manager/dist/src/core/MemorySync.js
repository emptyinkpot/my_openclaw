...
exports.syncExperienceToMemory = syncExperienceToMemory;
exports.syncAllExperiences = syncAllExperiences;
exports.searchRelatedExperiences = searchRelatedExperiences;
// 配置
const MEMORY_SYNC_ENABLED = process.env.EXPERIENCE_MEMORY_SYNC !== 'false';
/**
 * 将经验记录转换为记忆文本
 */
function experienceToMemoryText(record) {
    const parts = [
        `【经验记录】${record.title}`,
        ``,
        `类型: ${record.type}`,
 ...
 */
async function syncExperienceToMemory(record) {
 ...
 * 批量同步经验
 */
async function syncAllExperiences(records) {
    let successCount = 0;
 ...
 */
async function searchRelatedExperiences(query, limit = 5) {
    if (!MEMORY_SYNC_ENABLED) {
        return [];
    }
    try {
