...
export class MemoryService {
   ...
   * 存储经验到 Memory
   */
  async storeExperience(exp: ExperienceRecord): Promise<boolean> {
    if (!this.enabled) {
   ...
   * 语义搜索经验
   */
  async searchExperiences(query: string, limit: number = 10): Promise<MemoryRecallResult[]> {
    if (!this.enabled) {
      return [];
    }
    
    try {
      const params: MemoryRecallParams = {
        query,
        scope: 'experience',
        limit,
        minScore: 0.3,
   ...
   * 构建经验内容字符串
   */
  private buildExperienceContent(exp: ExperienceRecord): string {
    const parts = [
      `# ${exp.title}`,
      ``,
      `## 类型: ${exp.type}`,
   ...
   * HTTP 方式存储（备用）
   */
  private async httpStore(params: MemoryStoreParams): Promise<boolean> {
    try {
   ...
   * HTTP 方式检索（备用）
   */
  private async httpRecall(params: MemoryRecallParams): Promise<MemoryRecallResult[]> {
    try {
      ...
      const result = await response.json() as { results?: MemoryRecallResult[] };
      return result.results || [];
    } catch (e) {
   ...
   * 批量同步经验到 Memory
   */
  async syncAllExperiences(records: ExperienceRecord[]): Promise<{ synced: number; failed: number }> {
    let synced = 0;
    let failed = 0;
