...
export class FanqiePublisher {
     ...
     */
    getPendingChapters(workId_1) {
        return __awaiter(this, arguments, void 0, function* (workId, limit = 10) {
            const repo = getChapterRepository();
            const chapters = yield repo.getPendingPublish({ workId, limit });
     ...
     * 发布单章到番茄
     */
    publishChapter(chapter_1, account_1) {
        return __awaiter(this, arguments, void 0, function* (chapter, account, options = {}) {
            const { dryRun = false, headless = false, onProgress } = options;
            const emitProgress = (action, detail) => {
                logger.info(`  → ${action}`);
                onProgress === null || onProgress === void 0 ? void 0 : onProgress({ action, detail });
            };
            ...
            try {
                // 初始化浏览器
                emitProgress('启动浏览器...');
                yield this.initBrowser(account, headless);
                // 1. 在番茄找到对应作品
                emitProgress('查找作品...');
                const fanqieWork = yield this.findFanqieWork(chapter.workTitle);
                if (!fanqieWork) {
                ...
                // 2. 检查番茄最新章节
                emitProgress('检查章节状态...');
                const latestChapter = yield this.getFanqieLatestChapter(fanqieWork.id);
                ...
                // 4. 发布章节
                emitProgress('正在发布章节...');
                const result = yield this.doPublishToFanqie(fanqieWork.id, chapter);
                // 5. 更新数据库状态
                if (result.success) {
                    emitProgress('更新数据库状态...');
                    yield this.updatePublishStatus(chapter.workId, chapter.chapterNumber, 'published');
                    emitProgress('发布成功!');
                }
                else {
     ...
     * 初始化浏览器
     */
    initBrowser(account, headless) {
        return __awaiter(this, void 0, void 0, function* () {
            // 清理锁文件
            this.cleanBrowserLocks(account.browserDir);
            this.browserContext = yield chromium.launchPersistentContext(account.browserDir, {
                headless,
                channel: 'chrome',
                viewport: { width: 1400, height: 850 },
                args: ['--no-sandbox'],
            });
            this.page = this.browserContext.pages()[0] || (yield this.browserContext.newPage());
            // 如果有cookies文件，加载它
            if (fs.existsSync(account.cookiesFile)) {
                try {
                    const cookies = JSON.parse(fs.readFileSync(account.cookiesFile, 'utf-8'));
                    if (cookies.cookies) {
                        yield this.browserContext.addCookies(cookies.cookies);
                    }
                }
                catch (e) {
     ...
     * 在番茄找到对应作品
     */
    findFanqieWork(workTitle) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.page)
                return null;
            yield this.page.goto('https://fanqienovel.com/main/writer/book-manage', {
                waitUntil: 'domcontentloaded',
            });
            // 等待作品列表加载
            yield this.page.waitForSelector('[id^="long-article-table-item-"]', {
                state: 'visible',
                timeout: 10000,
            });
            // 关闭弹窗
            ...
            // 获取所有作品
            const works = yield this.page.evaluate(() => {
                ...
                return Array.from(items).map(item => {
                    ...
                    const title = titleEl ? (((_a = titleEl.textContent) === null || _a === void 0 ? void 0 : _a.split('\n')[0].trim()) || '') : '';
                    return { id, title };
                });
            });
            // 匹配作品（精确匹配或包含匹配）
     ...
     * 获取番茄作品的最新章节号
     */
    getFanqieLatestChapter(workId) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.page)
                return 0;
            // 进入章节管理
            const workCard = this.page.locator(`#long-article-table-item-${workId}`);
            yield workCard.hover();
            yield workCard.getByRole('button', { name: '章节管理' }).click();
            // 等待章节列表
            yield this.page.waitForSelector('table tbody tr', { state: 'visible' });
            yield this.closePopups();
            // 获取最新章节号
            const chapterTexts = yield this.page.evaluate(() => {
     ...
     * 执行发布
     */
    doPublishToFanqie(workId, chapter) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.page) {
            ...
            // 条件等待：等待"新建章节"按钮可见
            yield this.page.waitForSelector('a:has-text("新建章节"), button:has-text("新建章节")', {
                state: 'visible',
                timeout: 5000,
            }).catch(() => {
                // 如果超时继续执行
            ...
            // 保存截图用于调试
            yield this.page.screenshot({ path: '/tmp/fanqie-before-click.png' });
            logger.debug('    已保存点击前截图: /tmp/fanqie-before-click.png');
            // 调试：点击前的页面信息
            const beforeClick = yield this.page.evaluate(() => ({
                url: window.location.href,
                buttons: Array.from(document.querySelectorAll('button, a, [role="button"]')).slice(0, 15).map(b => {
                    var _a, _b;
                    return ({
                        text: (_a = b.textContent) === null || _a === void 0 ? void 0 : _a.slice(0, 30),
                        class: (_b = b.className) === null || _b === void 0 ? void 0 : _b.slice(0, 50),
                        tag: b.tagName,
                    });
                }),
            }));
            ...
            try {
                const href = yield this.page.evaluate(() => {
                    const link = Array.from(document.querySelectorAll('a')).find(a => { var _a; return ((_a = a.textContent) === null || _a === void 0 ? void 0 : _a.trim()) === '新建章节'; });
                ...
                if (href) {
                    ...
                    yield this.page.goto(fullUrl, { waitUntil: 'networkidle' });
            ...
            if (!clicked) {
                try {
                    ...
                    if ((yield linkBtn.count()) > 0 && (yield linkBtn.isVisible())) {
                        yield linkBtn.click({ timeout: 5000 });
            ...
            if (!clicked) {
                try {
                    ...
                    if ((yield btn.count()) > 0 && (yield btn.isVisible())) {
                        yield btn.click({ timeout: 5000 });
            ...
            if (!clicked) {
                try {
                    const result = yield this.page.evaluate(() => {
                        ...
                        const btn = Array.from(document.querySelectorAll('button')).find(b => { var _a; return ((_a = b.textContent) === null || _a === void 0 ? void 0 : _a.trim()) === '新建章节'; });
                        if (btn) {
                            btn.click();
            ...
            logger.info('    等待页面响应...');
            try {
                yield this.page.waitForSelector('input[type="text"], [contenteditable="true"]', {
                    state: 'visible',
                    timeout: 5000,
                });
            }
            ...
            // 保存点击后截图
            yield this.page.screenshot({ path: '/tmp/fanqie-after-click.png' });
            // 检查点击后的页面状态
            const afterClick = yield this.page.evaluate(() => ({
                url: window.location.href,
                ...
                buttons: Array.from(document.querySelectorAll('button')).slice(0, 5).map(b => { var _a; return (_a = b.textContent) === null || _a === void 0 ? void 0 : _a.slice(0, 15); }),
            }));
            ...
            if (afterClick.inputs === 0 && afterClick.editors === 0) {
                ...
                try {
                    ...
                    if (yield manageBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
                        yield manageBtn.click();
                        logger.debug(`    点击"章节管理"按钮`);
                        // 条件等待章节列表加载
                        yield this.page.waitForSelector('table tbody tr, .chapter-list, [class*="chapter"]', {
                            state: 'visible',
                            timeout: 5000,
                        }).catch(() => { });
                    }
                ...
                // 再次尝试点击新建章节
                const retryResult = yield this.page.evaluate(() => {
                    const btn = Array.from(document.querySelectorAll('button, a')).find(b => { var _a, _b; return ((_a = b.textContent) === null || _a === void 0 ? void 0 : _a.includes('新建章节')) || ((_b = b.textContent) === null || _b === void 0 ? void 0 : _b.includes('创建章节')); });
                    if (btn) {
                        btn.click();
            ...
            // 调试：获取页面上的所有输入框信息
            const inputDebug = yield this.page.evaluate(() => {
                ...
                return Array.from(inputs).slice(0, 10).map((el, i) => {
                    ...
                    return ({
                        ...
                        name: el.name || '',
                        className: ((_a = el.className) === null || _a === void 0 ? void 0 : _a.slice(0, 50)) || '',
                        value: ((_b = el.value) === null || _b === void 0 ? void 0 : _b.slice(0, 30)) || '',
                    });
                });
            });
            ...
            // 先获取所有输入框的详细信息
            const allInputs = yield this.page.evaluate(() => {
            ...
            if (numberInputs.length > 0) {
                ...
                yield input.fill(chapter.chapterNumber.toString());
            ...
            if (!chapterNumFilled) {
                ...
                for (const selector of chapterNumSelectors) {
                    ...
                    if (yield input.isVisible().catch(() => false)) {
                        yield input.fill(chapter.chapterNumber.toString());
            ...
            if (!chapterNumFilled) {
                ...
                const verifyChapterNum = yield this.page.evaluate(() => {
                    const input = document.querySelector('input');
                    return (input === null || input === void 0 ? void 0 : input.value) || null;
                });
            ...
            for (const selector of titleSelectors) {
                const input = this.page.locator(selector).first();
                if (yield input.isVisible().catch(() => false)) {
                    yield input.fill(chapter.chapterTitle);
            ...
            if (!titleFilled) {
                ...
                const count = yield inputs.count();
                if (count > 1) {
                    yield inputs.nth(1).fill(chapter.chapterTitle);
                }
            }
            // 填充正文内容
            ...
            for (const selector of editorSelectors) {
                ...
                if (yield editor.isVisible().catch(() => false)) {
            ...
            if (!editorFilled) {
                ...
                if (yield fallbackEditor.isVisible().catch(() => false)) {
                    yield fallbackEditor.fill(cleanedContent);
                }
                else {
            ...
            // 验证填入内容
            const verifyContent = yield this.page.evaluate(() => {
                var _a;
                const editor = document.querySelector('[contenteditable="true"], textarea');
                return ((_a = editor === null || editor === void 0 ? void 0 : editor.textContent) === null || _a === void 0 ? void 0 : _a.slice(0, 50)) || '';
            });
            logger.debug(`    验证填入内容: ${verifyContent}`);
            // 等待自动保存（条件等待）
            logger.info('  → 等待自动保存...');
            yield this.waitForAutoSave(10000);
            // 点击下一步/发布
            logger.info('  → 进入发布流程...');
            // Step 1: 点击"下一步"
            try {
                const nextBtn = this.page.locator('button').filter({ hasText: /^下一步$/ }).first();
                yield nextBtn.click({ timeout: 5000 });
                logger.debug('    点击"下一步"成功');
                // 条件等待：等待"提交"按钮出现
                yield this.page.locator('button').filter({ hasText: /^提交$/ }).first().waitFor({ state: 'visible', timeout: 3000 }).catch(() => { });
            }
            catch (e) {
            ...
            try {
                ...
                yield submitBtn.click({ timeout: 5000 });
                logger.debug('    点击"提交"成功');
                // 条件等待：等待弹窗或"确认发布"按钮出现
                yield this.page.locator('button').filter({ hasText: /^确认发布$/ }).first().waitFor({ state: 'visible', timeout: 3000 }).catch(() => { });
            }
            catch (e) {
                logger.warn(`    点击"提交"失败: ${e.message}`);
            }
            // Step 3: 处理弹窗（取消/我知道了）- 使用条件等待
            const hasDialog = yield this.waitForDialog(5000);
            if (hasDialog) {
                logger.debug('    检测到弹窗');
                const dialogButtons = yield this.page.locator('div[class*="modal"] button, div[class*="dialog"] button, div[role="dialog"] button').all();
                logger.debug(`    检测到 ${dialogButtons.length} 个弹窗按钮`);
                if (dialogButtons.length > 0) {
                    for (const btn of dialogButtons) {
                        const text = yield btn.innerText().catch(() => '');
                        logger.debug(`      弹窗按钮: "${text.trim()}"`);
                    }
                    // 尝试点击"取消"或"我知道了"
                    try {
                        const cancelBtn = this.page.locator('button').filter({ hasText: /^取消$/ }).first();
                        yield cancelBtn.click({ timeout: 3000 });
                        logger.debug('    点击"取消"关闭弹窗');
                    }
                    catch (e) {
                        try {
                            const knowBtn = this.page.locator('button').filter({ hasText: /^我知道了$/ }).first();
                            yield knowBtn.click({ timeout: 3000 });
                    ...
                    // 条件等待：等待弹窗消失
                    yield this.waitForDialogClosed(3000);
                }
            }
            else {
                logger.debug('    未检测到弹窗');
            ...
            try {
                ...
                if ((yield freeRadio.count()) > 0) {
                    yield freeRadio.click();
            ...
            while (retryCount < 3 && !publishSuccess) {
                try {
                    ...
                    yield confirmBtn.click({ timeout: 10000 });
            ...
            // 条件等待：检测页面跳转
            logger.debug('    等待发布完成...');
            yield this.waitForNavigation('chapter-manage', 8000);
            // 检查结果
            const pageInfo = yield this.page.evaluate(() => {
                var _a;
                return ({
                    url: window.location.href,
                    hasSuccessToast: !!document.querySelector('[class*="success"], [class*="Success"]'),
                    pageText: ((_a = document.body.textContent) === null || _a === void 0 ? void 0 : _a.slice(0, 500)) || '',
                });
            });
            ...
            const success = yield this.page.evaluate(() => {
                ...
                if (successElements.length > 0) {
                    ...
                    for (const el of Array.from(successElements)) {
                        if (((_a = el.textContent) === null || _a === void 0 ? void 0 : _a.includes('发布')) || ((_b = el.textContent) === null || _b === void 0 ? void 0 : _b.includes('成功'))) {
                            return true;
                        }
                    }
                }
            ...
            if (success) {
                ...
                yield this.page.screenshot({ path: screenshotPath, fullPage: true });
    ...
    waitForAutoSave() {
        return __awaiter(this, arguments, void 0, function* (timeout = 10000) {
            ...
            while (Date.now() - startTime < timeout) {
                // 检查是否已保存（多种可能的提示文本）
                const isSaved = yield this.page.evaluate(() => {
                    const pageText = document.body.textContent || '';
                    // 检查保存完成提示
    ...
    waitForDialog() {
        return __awaiter(this, arguments, void 0, function* (timeout = 5000) {
            ...
            try {
                // 等待弹窗元素出现
                yield this.page.waitForSelector('.arco-modal-wrapper, .ant-modal, [role="dialog"], .modal, div[class*="modal"], div[class*="dialog"]', {
                    state: 'visible',
                    timeout: timeout,
                });
                return true;
     ...
     * 条件等待：等待页面跳转
     */
    waitForNavigation(targetUrlPattern_1) {
        return __awaiter(this, arguments, void 0, function* (targetUrlPattern, timeout = 10000) {
            if (!this.page)
                return false;
            const startTime = Date.now();
            ...
            yield this.page.evaluate(() => {
                ...
                for (const btn of Array.from(buttons)) {
                    for (const text of texts) {
                        if (((_a = btn.textContent) === null || _a === void 0 ? void 0 : _a.includes(text)) && btn instanceof HTMLElement) {
                            btn.click();
                            break;
                        }
                    }
    ...
    cleanContent(content) {
        ...
        const filtered = lines.filter(line => {
            ...
            if (!trimmed)
                return true;
            if (junkLines.some(j => trimmed.includes(j)))
                return false;
    ...
    updatePublishStatus(workId, chapterNumber, status) {
        return __awaiter(this, void 0, void 0, function* () {
            const repo = getChapterRepository();
            yield repo.updatePublishStatus(workId, chapterNumber, status);
        });
    }
    /**
     * 获取番茄账号列表
