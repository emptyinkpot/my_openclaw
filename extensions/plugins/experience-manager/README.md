...
The experience module can optionally mirror data to a cloud HTTP endpoint.
Local JSON remains the source of truth. Cloud sync is best-effort and non-blocking.

### Recommended form

Use a normalized envelope for both records and notes:
...
```json
{
  ...
  "capturedAtIso": "2026-03-24T22:00:00.000Z",
  "payload": {
    "title": "Example title",
    "type": "bug_fix",
    ...
    "verification": ["curl /health", "DOM check"]
  }
}
```
...
- `EXPERIENCE_CLOUD_SYNC=true`
- `EXPERIENCE_CLOUD_SYNC_ENDPOINT=https://your-cloud.example.com`
- `EXPERIENCE_CLOUD_SYNC_PATH_PREFIX=/api/experience-cloud`
- `EXPERIENCE_CLOUD_SYNC_TOKEN=...`
- `EXPERIENCE_CLOUD_SYNC_NAMESPACE=experience-manager`
- `EXPERIENCE_CLOUD_SYNC_TIMEOUT_MS=8000`
- `EXPERIENCE_CLOUD_SYNC_NOTES=true`
...
- Keep local JSON as the primary store.
- Treat cloud sync as append/upsert backup.
- Include `source` and `verification` in meaningful experiences.
- Keep future UI forms aligned with the normalized envelope above.
