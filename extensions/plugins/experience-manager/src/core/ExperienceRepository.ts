...
import { syncExperienceToCloud } from './CloudSync';
...
export class ExperienceRepository {
  ...
  create(record: Omit<ExperienceRecord, 'id' | 'timestamp'>): ExperienceRecord {
    ...
    syncExperienceToCloud(newRecord, 'upsert').catch(err => {
      console.warn('[Experience] 同步到云端失败:', err.message);
    });
  ...
  delete(id: string): boolean {
    ...
    if (removed) {
      syncExperienceToCloud(removed, 'delete').catch(err => {
        console.warn('[Experience] 删除同步到云端失败:', err.message);
      });
    }
    return true;
  }
  ...
  update(id: string, updates: Partial<Omit<ExperienceRecord, 'id' | 'timestamp'>>): ExperienceRecord | null {
    ...
    this.save();
    syncExperienceToCloud(this.data.records[index], 'upsert').catch(err => {
      console.warn('[Experience] 更新同步到云端失败:', err.message);
    });
