...
// ==================== 自己实现 ContentCraftAutoService ====================
interface AutoServiceStatus {
  running: boolean;
...
class SimpleContentCraftAutoService {
  ...
  updateConfig(newConfig: Partial<AutoServiceConfig>): void {
    ...
    console.log('[SimpleContentCraftAutoService] 配置已更新:', this.config);
  ...
  start(): void {
    if (this.running) {
      console.warn('[SimpleContentCraftAutoService] 服务已在运行中');
      return;
    }

    this.running = true;
    this.config.enabled = true;
    console.log('[SimpleContentCraftAutoService] 自动处理服务已启动');
    
    const activityLog = getActivityLog();
    activityLog.log('system', '🚀 Content Craft 自动处理服务已启动');
  ...
  stop(): void {
    if (!this.running) {
      console.warn('[SimpleContentCraftAutoService] 服务未在运行');
      return;
    }
    ...
    this.stopTimer();
    console.log('[SimpleContentCraftAutoService] 自动处理服务已停止');
  }
  ...
  private startTimer(): void {
    ...
    console.log(`[SimpleContentCraftAutoService] 定时器已启动，间隔: ${this.config.processInterval}秒`);
  }
  ...
  private async processChapters(): Promise<void> {
    ...
    if (this.isProcessing) {
      console.log('[SimpleContentCraftAutoService] 上次处理尚未完成，跳过本次执行');
      return;
    }
    ...
    try {
      ...
      console.log('[SimpleContentCraftAutoService] 开始处理章节...');
      activityLog.log('progress', 'Content Craft 开始检查待处理章节...');
      ...
      if (chapters.length === 0) {
        console.log('[SimpleContentCraftAutoService] 没有需要处理的章节');
        activityLog.log('progress', '没有需要处理的章节，等待中...');
        this.currentTask = null;
        return;
      }

      console.log(`[SimpleContentCraftAutoService] 找到 ${chapters.length} 个需要处理的章节`);
      activityLog.log('progress', `找到 ${chapters.length} 个需要处理的章节`);
      ...
      for (const chapter of chapters) {
        ...
        try {
          ...
          console.log(`[SimpleContentCraftAutoService] ${this.currentTask}`);
          
          if ((chapter.state || chapter.status) === 'outline') {
            activityLog.log('generating', `开始生成第 ${chapter.chapter_number} 章内容`);
            console.log(`[SimpleContentCraftAutoService] ===== 开始生成第 ${chapter.chapter_number} 章 =====`);
            
            const generator = new GenerationPipeline();
            const result = await generator.generateFromDatabase({
              workId: chapter.work_id,
              chapterNumber: chapter.chapter_number,
              relatedChapterCount: this.config.relatedChapterCount,
              settings: { autoPolish: this.config.autoPolish }
            }, (progress: any) => {
              console.log(`[SimpleContentCraftAutoService] [生成进度] ${progress.phase || 'generating'}: ${progress.message} (${progress.progress}%)`);
              if (progress.message) {
                activityLog.log('progress', `[生成] ${progress.message}`);
              }
            });
            ...
            if (result.text) {
              ...
              console.log(`[SimpleContentCraftAutoService] 已更新章节 ${chapter.id} 的内容`);
            }
            
            console.log(`[SimpleContentCraftAutoService] ===== 第 ${chapter.chapter_number} 章生成完成 =====`);
            activityLog.log('completed', `第 ${chapter.chapter_number} 章生成完成`);
          } else if ((chapter.state || chapter.status) === 'first_draft') {
            activityLog.log('polishing', `开始润色第 ${chapter.chapter_number} 章内容`);
            console.log(`[SimpleContentCraftAutoService] ===== 开始润色第 ${chapter.chapter_number} 章 =====`);
            ...
            const result = await polishPipeline.execute({
              ...
              console.log(`[SimpleContentCraftAutoService] [润色进度] ${progress.currentStep || 'processing'}: ${progress.message} (${progress.progress}%)`);
              if (progress.message) {
                activityLog.log('progress', `[润色] ${progress.message}`);
              }
            });
            ...
            if (result.text) {
              ...
              console.log(`[SimpleContentCraftAutoService] 已更新章节 ${chapter.id} 的润色内容`);
            }
            
            console.log(`[SimpleContentCraftAutoService] ===== 第 ${chapter.chapter_number} 章润色完成 =====`);
            activityLog.log('completed', `第 ${chapter.chapter_number} 章润色完成`);
          }
          
          this.processedCount++;
          console.log(`[SimpleContentCraftAutoService] 章节处理成功: ${chapter.title || chapter.chapter_number}`);
        } catch (error: any) {
          this.errorCount++;
          console.error(`[SimpleContentCraftAutoService] 章节处理失败: ${chapter.title || chapter.chapter_number}`, error.message);
        }
      }
      
      this.currentTask = null;
      console.log('[SimpleContentCraftAutoService] 章节处理完成');
    } catch (error: any) {
      this.errorCount++;
      console.error('[SimpleContentCraftAutoService] 处理过程出错:', error.message);
      this.currentTask = null;
    } finally {
...
let simpleContentCraftAutoServiceInstance: SimpleContentCraftAutoService | null = null;

function getContentCraftAutoService(): SimpleContentCraftAutoService {
  if (!simpleContentCraftAutoServiceInstance) {
    simpleContentCraftAutoServiceInstance = new SimpleContentCraftAutoService();
  }
  return simpleContentCraftAutoServiceInstance;
}

// ==================== 自己实现 AuditAutoService ====================
interface AuditAutoServiceStatus {
  running: boolean;
  lastRunTime: string | null;
  currentTask: string | null;
  processedCount: number;
...
interface AuditAutoServiceConfig {
  enabled: boolean;
  processInterval: number;
  maxChaptersPerRun: number;
  autoFix: boolean;
...
class SimpleAuditAutoService {
  ...
  updateConfig(newConfig: Partial<AuditAutoServiceConfig>): void {
    this.config = { ...this.config, ...newConfig };
    console.log('[SimpleAuditAutoService] 配置已更新:', this.config);
  ...
  start(): void {
    if (this.running) {
      console.warn('[SimpleAuditAutoService] 服务已在运行中');
      return;
    }

    this.running = true;
    this.config.enabled = true;
    console.log('[SimpleAuditAutoService] 自动审核服务已启动');
    
    const activityLog = getActivityLog();
    activityLog.log('system', '✅ 审核自动处理服务已启动');
  ...
  stop(): void {
    if (!this.running) {
      console.warn('[SimpleAuditAutoService] 服务未在运行');
      return;
    }
    ...
    this.config.enabled = false;
    this.stopTimer();
    console.log('[SimpleAuditAutoService] 自动审核服务已停止');
  }
  ...
  private startTimer(): void {
    ...
    console.log(`[SimpleAuditAutoService] 定时器已启动，间隔: ${this.config.processInterval}秒`);
  }
  ...
  private async processChapters(): Promise<void> {
    if (!this.running) return;
    if (this.isProcessing) {
      console.log('[SimpleAuditAutoService] 上次处理尚未完成，跳过本次执行');
      return;
    }
    ...
    try {
      this.isProcessing = true;
      this.lastRunTime = new Date();
      console.log('[SimpleAuditAutoService] 开始处理章节审核...');
      activityLog.log('progress', '审核服务开始检查待审核章节...');
      ...
      if (chapters.length === 0) {
        console.log('[SimpleAuditAutoService] 没有需要审核的章节');
        activityLog.log('progress', '没有需要审核的章节，等待中...');
        this.currentTask = null;
        return;
      }

      console.log(`[SimpleAuditAutoService] 找到 ${chapters.length} 个需要审核的章节`);
      activityLog.log('progress', `找到 ${chapters.length} 个需要审核的章节`);
      ...
      for (const chapter of chapters) {
        ...
        try {
          ...
          console.log(`[SimpleAuditAutoService] ${this.currentTask}`);
          
          activityLog.log('auditing', `开始审核第 ${chapter.chapter_number} 章内容`);
          console.log(`[SimpleAuditAutoService] ===== 开始审核第 ${chapter.chapter_number} 章 =====`);
          ...
          console.log(`[SimpleAuditAutoService] ===== 第 ${chapter.chapter_number} 章审核完成 =====`);
          activityLog.log('completed', `第 ${chapter.chapter_number} 章审核完成`);
          
          this.processedCount++;
          console.log(`[SimpleAuditAutoService] 章节审核成功: ${chapter.title || chapter.chapter_number}`);
        } catch (error: any) {
          this.errorCount++;
          console.error(`[SimpleAuditAutoService] 章节审核失败: ${chapter.title || chapter.chapter_number}`, error.message);
        }
      }
      
      this.currentTask = null;
      console.log('[SimpleAuditAutoService] 章节审核处理完成');
    } catch (error: any) {
      this.errorCount++;
      console.error('[SimpleAuditAutoService] 审核处理过程出错:', error.message);
      this.currentTask = null;
    } finally {
      this.isProcessing = false;
    }
...
let simpleAuditAutoServiceInstance: SimpleAuditAutoService | null = null;

function getAuditAutoService(): SimpleAuditAutoService {
  if (!simpleAuditAutoServiceInstance) {
    simpleAuditAutoServiceInstance = new SimpleAuditAutoService();
  }
  return simpleAuditAutoServiceInstance;
}
...
/**
 * 从数据库生成章节内容（完整流程：生成 + 润色）
 */
async function generateChapterFromDb(workId: number, chapterNumber: number): Promise<any> {
  console.log('[ContentCraft] 从数据库生成，workId:', workId, 'chapter:', chapterNumber);
  ...
  const result = await generator.generateFromDatabase({
    ...
    console.log(`[ContentCraft] [生成进度] ${progress.phase || 'generating'}: ${progress.message} (${progress.progress}%)`);
  });
 ...
 * 从数据库读取章节内容并润色
 */
async function polishChapterFromDb(workId: number, chapterNumber: number): Promise<any> {
  console.log('[ContentCraft] 从数据库读取并润色，workId:', workId, 'chapter:', chapterNumber);
  ...
  const result = await polishPipeline.execute({
    ...
    console.log(`[ContentCraft] [润色进度] ${progress.currentStep || 'processing'}: ${progress.message} (${progress.progress}%)`);
  });
  ...
  if (result.text) {    
    ...
    try {
      ...
      if (colCheck[0].cnt === 0) {
        ...
        `);
        console.log('[ContentCraft] 已添加 polish_info 字段');
      }
      ...
      console.warn('[ContentCraft] 更新 polish_info 失败（可能字段不存在）:', e);
    }
...
// 导出这些函数供 ContentCraftAutoService 使用
export { generateChapterFromDb, polishChapterFromDb };
