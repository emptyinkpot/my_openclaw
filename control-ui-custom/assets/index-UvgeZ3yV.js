...
;(() => {
  if (window.__OPENCLAW_CONTROL_BOOTSTRAP_INSTALLED__) return;
  window.__OPENCLAW_CONTROL_BOOTSTRAP_INSTALLED__ = true;
  ...
  let token = (params.get("token") || hashParams.get("token") || seededName.token || storedToken || "").trim();
  const gatewayUrl = (params.get("gatewayUrl") || hashParams.get("gatewayUrl") || seededName.gatewayUrl || `ws://${location.host}`)
    .trim()
    .replace(/\/+$/, "");
  ...
  window.__OPENCLAW_CONTROL_BOOTSTRAP_DEBUG__ = debug;
  try {
    if (seededName && typeof seededName === "object" && window.name) {
      window.name = "";
    }
  } catch {
    // ignore
  ...
  try {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        chatFocusMode:
          typeof existing.chatFocusMode === "boolean" ? existing.chatFocusMode : false,
        chatShowThinking:
          typeof existing.chatShowThinking === "boolean" ? existing.chatShowThinking : true,
  ...
  const autoConfirmChangeGateway = () => {
    ...
    const confirmButton = Array.from(document.querySelectorAll(".exec-approval-card button")).find(
  ...
  const navBarMarkup = `
    ...
    <div class="nav-bar">
      <div class="nav-brand">
        ...
        <div class="nav-title">OpenClaw</div>
      </div>
      <a href="javascript:void(0)" data-action="settings">设置</a>
      <a href="/novel/">小说管理</a>
      <a href="/automation">自动�?/a>
      <a href="/project-structure.html">项目结构</a>
      <a href="/experience.html">经验</a>
      <a href="/cache.html">缓存</a>
      <a href="/logs.html">日志</a>
      <a href="/cache-manage.html">缓存管理</a>
      <a href="/file-manager">文件管理</a>
      <a href="/control-ui-custom/launch.html">原生界面</a>
    </div>`;
  ...
  const seedNavBar = () => {
    ...
    const applyLayout = () => {
      ...
      debug.navBarSeeded = true;
    };
    container.innerHTML = navBarMarkup;
    applyLayout();
  };

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", seedNavBar, { once: true });
  } else {
    seedNavBar();
  }
  ...
  const seedLoginGate = () => {
    const app = document.querySelector("openclaw-app");
    if (!app) return false;
    const loginGate = app.querySelector(".login-gate");
    if (!loginGate) return false;
    const wsInput = loginGate.querySelector('input[placeholder="ws://127.0.0.1:18789"]') || loginGate.querySelector('input[type="text"]');
    const tokenInput = loginGate.querySelector('input[placeholder="OPENCLAW_GATEWAY_TOKEN (可�?"]') || loginGate.querySelector('input[type="password"]');
    const connectButton = loginGate.querySelector(".login-gate__connect");
    if (!wsInput || !tokenInput || !connectButton) return false;
    if (wsInput && wsInput.value !== gatewayUrl) {
      wsInput.value = gatewayUrl;
      wsInput.dispatchEvent(new Event("input", { bubbles: true, composed: true }));
    ...
    if (connectButton && token && !window.__OPENCLAW_CONTROL_AUTO_CONNECT_ATTEMPTED__) {
      window.__OPENCLAW_CONTROL_AUTO_CONNECT_ATTEMPTED__ = true;
      window.setTimeout(() => connectButton.click(), 100);
    }
    return false;
  };
  ...
  const loginTimer = window.setInterval(() => {
    loginAttempts += 1;
    const settled = seedLoginGate();
    if (settled || loginAttempts >= 80) {
      window.clearInterval(loginTimer);
    }
  }, 250);

  if (/\/launch\.html(?:$|[?#/])/.test(location.pathname)) {
    window.setTimeout(() => {
      const next = new URL(location.href);
      next.pathname = next.pathname.replace(/\/launch\.html(?:\/.*)?$/, "/index.html");
      next.search = "";
      history.replaceState({}, "", next);
    }, 0);
  }
