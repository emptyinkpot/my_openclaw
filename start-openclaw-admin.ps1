...
if (-not $ConfigPath) {
  $ConfigPath = Join-Path $PSScriptRoot 'projects\openclaw.json'
  if (-not (Test-Path -LiteralPath $ConfigPath)) {
    $ConfigPath = Join-Path $PSScriptRoot 'projects\openclaw.example.json'
  }
}
...
function Invoke-MainLauncher {
  ...
  & (Join-Path $PSScriptRoot 'start-openclaw.bat')
  exit $LASTEXITCODE
}
